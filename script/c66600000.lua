...
function card_entity.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    aux.PlayFieldSpell(tg1:GetFirst(),e,1-tp,eg,ep,ev,re,r,rp)
end 
...
function card_entity.joydspfilter(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsSetCard(0x131) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

function card_entity.joydsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if e:GetHandler():GetSequence()<5 then ft=ft+1 end
	if chk==0 then return ft>0 and Duel.IsExistingMatchingCard(card_entity.joydspfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end

function card_entity.joydspop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(tp,card_entity.joydspfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
    if #g>0 then 
